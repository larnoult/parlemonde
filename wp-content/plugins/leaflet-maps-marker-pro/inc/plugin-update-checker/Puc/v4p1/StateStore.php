...
if ( !class_exists('MMPPuc_v4p1_StateStore', false) ):

	class MMPPuc_v4p1_StateStore {
		/**
		...
		/**
		 * @var MMPPuc_v4p1_Update|null Cached update.
		 */
		...
		/**
		 * @return null|MMPPuc_v4p1_Update
		 */
		 ...
		 * @return $this
		 */
		public function setUpdate(MMPPuc_v4p1_Update $update = null) {
			$this->lazyLoad();
			$this->update = $update;
			return $this;
		}
		...
		public function save() {
			...
			if ( isset($this->update)) {
				...
				if ( MMPPuc_v4p1_Utils::startsWith($updateClass, $prefix) ) {
		...
		protected function load() {
			...
			$this->lastCheck = intval(MMPPuc_v4p1_Utils::get($state, 'lastCheck', 0));
			$this->checkedVersion = MMPPuc_v4p1_Utils::get($state, 'checkedVersion', '');
			$this->update = null;
