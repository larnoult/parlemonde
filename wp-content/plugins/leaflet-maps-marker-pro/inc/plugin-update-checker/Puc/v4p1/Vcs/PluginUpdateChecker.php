<?php
if ( !class_exists('MMPPuc_v4p1_Vcs_PluginUpdateChecker') ):

	class MMPPuc_v4p1_Vcs_PluginUpdateChecker extends MMPPuc_v4p1_Plugin_UpdateChecker implements MMPPuc_v4p1_Vcs_BaseChecker {
		/**
		...
		/**
		 * MMPPuc_v4p1_Vcs_PluginUpdateChecker constructor.
		 *
		...
		public function requestInfo($unusedParameter = null) {
			...
			$info = new MMPPuc_v4p1_Plugin_Info();
			...
			if ( empty($info->sections['changelog']) ) {
				...
				if ( empty($info->sections['changelog']) ) {
					$info->sections['changelog'] = __('There is no changelog available.', 'lmm');
				}
			}
		 ...
		 * @param MMPPuc_v4p1_Plugin_Info $pluginInfo
		 */
