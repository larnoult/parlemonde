...
if ( !class_exists('MMPPuc_v4p1_UpdateChecker', false) ):

	abstract class MMPPuc_v4p1_UpdateChecker {
		...
		/**
		 * @var MMPPuc_v4p1_Scheduler
		 */
		public $scheduler;

		/**
		 * @var MMPPuc_v4p1_UpgraderStatus
		 */
		protected $upgraderStatus;

		/**
		 * @var MMPPuc_v4p1_StateStore
		 */
		...
		public function __construct($metadataUrl, $directoryName, $slug = null, $checkPeriod = 12, $optionName = '') {
			...
			$this->upgraderStatus = new MMPPuc_v4p1_UpgraderStatus();
			$this->updateState = new MMPPuc_v4p1_StateStore($this->optionName);

			/* RH - moved to lmm text domain
			if ( did_action('init') ) {
				$this->loadTextDomain();
			} else {
		...
		*/
		 ...
		 *
		 * @return MMPPuc_v4p1_Update|null
		 */
		 ...
		 *
		 * @return MMPPuc_v4p1_Update An instance of Update, or NULL when no updates are available.
		 */
		...
		public function injectUpdate($updates) {
			...
			//RH only inject if license is valid
			if ( !empty($update) && lmm_validate_license_info() ) {
		 ...
		 * @return array [MMPPuc_v4p1_Metadata|null, array|WP_Error] A metadata instance and the value returned by wp_remote_get().
		 */
		protected function requestMetadata($metaClass, $filterRoot, $queryArgs = array()) {
			...
			//RH fallback to check via http only
		...
		protected function createDebugBarExtension() {
			return new MMPPuc_v4p1_DebugBar_Extension($this);
		}

		/**
		 * Display additional configuration details in the Debug Bar panel.
		 *
		 * @param MMPPuc_v4p1_DebugBar_Panel $panel
		 */
