...
if (basename($_SERVER['SCRIPT_FILENAME']) == 'leaflet-geositemap.php') {
	...
	wp_redirect(MMP_Globals::translate_permalink(MMP_Rewrite::get_base_url() . MMP_Rewrite::get_slug() . '/geositemap/'), 301);
	exit;
}
...
if (!lmm_is_plugin_active('leaflet-maps-marker-pro/leaflet-maps-marker.php') ) {
	...
	foreach ($markers as $marker) {
		...
		echo '<url>'.PHP_EOL;
		echo '<loc>' . MMP_Globals::translate_permalink($lmm_base_url . $lmm_slug . '/kml/marker/' . $marker['mid'] . '/') . '</loc>'.PHP_EOL;
	...
	foreach ($layers as $layer) {
		...
		echo '<url>'.PHP_EOL;
		echo '<loc>' . MMP_Globals::translate_permalink($lmm_base_url . $lmm_slug . '/kml/layer/' . $layer['lid'] . '/') . '</loc>'.PHP_EOL;
		echo '<lastmod>' . date("Y-m-d", $date_kml) . '</lastmod>'.PHP_EOL;
		echo '</url>'.PHP_EOL;
	}
	echo '</urlset>';
