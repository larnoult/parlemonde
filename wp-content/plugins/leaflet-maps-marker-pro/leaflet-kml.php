...
if (basename($_SERVER['SCRIPT_FILENAME']) == 'leaflet-kml.php') {
	...
	if (isset($_GET['layer'])) {
		...
		wp_redirect(MMP_Globals::translate_permalink(MMP_Rewrite::get_base_url() . MMP_Rewrite::get_slug() . '/kml/layer/' . $layer . '/' . $argv), 301);
	} elseif (isset($_GET['marker'])) {
		$marker = esc_html($_GET['marker']);
		unset($_GET['marker']);
		$argv = (!empty($_GET)) ? '?' . http_build_query($_GET) : '';
		wp_redirect(MMP_Globals::translate_permalink(MMP_Rewrite::get_base_url() . MMP_Rewrite::get_slug() . '/kml/marker/' . $marker . '/' . $argv), 301);
	}
	exit;
}
...
if (!lmm_is_plugin_active('leaflet-maps-marker-pro/leaflet-maps-marker.php') ) {
	...
	if (get_query_var('layer', false)) {
		...
		$sql = 'SELECT m.id as mid, m.markername as mmarkername, m.layer as mlayer, m.icon as micon, m.createdby as mcreatedby, m.createdon as mcreatedon, m.lat as mlat, m.lon as mlon, m.popuptext as mpopuptext, m.kml_timestamp as mkml_timestamp, m.address as maddress, l.createdby as lcreatedby, l.createdon as lcreatedon, l.name as lname, l.wms as lwms, l.wms2 as lwms2, l.wms3 as lwms3, l.wms4 as lwms4, l.wms5 as lwms5, l.wms6 as lwms6, l.wms7 as lwms7, l.wms8 as lwms8, l.wms9 as lwms9, l.wms10 as lwms10 FROM `'.$table_name_markers.'` AS m INNER JOIN `'.$table_name_layers.'` AS l ON m.layer LIKE concat(\'%"\',l.id,\'"%\') '.$q.' GROUP BY m.id';
		...
		if (empty($markers)) {
			...
			$layername_wpml = MMP_Globals::translate_single_string($layername, 'Layer (ID ' . intval(get_query_var('layer')) . ') name');
			if (get_query_var('layer') != 'all') {
				echo '<Folder>'.PHP_EOL;
				echo '<name>' . htmlspecialchars($layername_wpml) . '</name>'.PHP_EOL;
			}

			foreach ($markers as $marker) {
				if (isset($_GET['markername']) && $_GET['markername'] == 'show') {
					$name = stripslashes(htmlspecialchars($marker['mmarkername']));
					$name_popup = '';
				} else if (isset($_GET['markername']) && $_GET['markername'] == 'hide') {
					$name = '';
					$name_popup = '';
				} else if (isset($_GET['markername']) && $_GET['markername'] == 'popup') {
					$name = '';
		...
		$sql = 'SELECT m.layer as mlayer,m.icon as micon,m.popuptext as mpopuptext,m.id as mid,m.markername as mmarkername,m.createdby as mcreatedby, m.createdon as mcreatedon, m.wms as mwms, m.wms2 as mwms2, m.wms3 as mwms3, m.wms4 as mwms4, m.wms5 as mwms5, m.wms6 as mwms6, m.wms7 as mwms7, m.wms8 as mwms8, m.wms9 as mwms9, m.wms10 as mwms10, m.lat as mlat, m.lon as mlon, m.kml_timestamp as mkml_timestamp, m.address as maddress FROM `'.$table_name_markers.'` AS m LEFT OUTER JOIN `'.$table_name_layers.'` AS l ON m.layer LIKE concat(\'%"\',l.id,\'"%\') '.$q.' GROUP BY m.id';
		$markers = $wpdb->get_results($sql, ARRAY_A);
		//info: prepare WPML supported strings
		if ($ml_checked = MMP_Globals::check_multilingual()) {
			foreach ($markers as $key => $marker) {
