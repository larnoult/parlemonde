...
<div id="fieldmaster-upgrade-wrap" class="wrap">
	...
	<p><?php echo sprintf( __("The following sites require a DB upgrade. Check the ones you want to update and then click %s.", 'fieldmaster'), '"' . $button . '"'); ?></p>
	...
	<table class="wp-list-table widefat">
		...
		<thead>
			<tr>
				...
				<th class="manage-column" scope="col" style="width:33%;"><label for="sites-select-all"><?php _e("Site", 'fieldmaster'); ?></label></th>
				<th><?php _e("Description", 'fieldmaster'); ?></th>
			</tr>
		</thead>
		...
		<tr>
			...
			<th class="manage-column" scope="col"><label for="sites-select-all-2"><?php _e("Site", 'fieldmaster'); ?></label></th>
			<th><?php _e("Description", 'fieldmaster'); ?></th>
		</tr>
		</tfoot>
		...
		<?php foreach( $sites as $i => $site ): ?>
			...
			<tr<?php if( $i % 2 == 0 ): ?> class="alternate"<?php endif; ?>>
				...
				<td>
				<?php if( $site['updates'] ): ?>
					<span class="response"><?php printf(__('Site requires database upgrade from %s to %s', 'fieldmaster'), $site['fieldmaster_version'], $plugin_version); ?></span>
				<?php else: ?>
					<?php _e("Site is up to date", 'fieldmaster'); ?>
				<?php endif; ?>
				</td>
			</tr>
	...
	<p class="show-on-complete"><?php echo sprintf( __('Database Upgrade complete. <a href="%s">Return to network dashboard</a>', 'fieldmaster'), network_admin_url() ); ?></p>
		...
		var upgrader = {
			...
			upgrade: function(){
				...
				var $input = this.$inputs.eq( this.i ),
					$tr = $input.closest('tr'),
					text = '<?php _e('Upgrade complete', 'fieldmaster'); ?>';
				
				
				// add loading
				$tr.find('.response').html('<i class="fieldmaster-loading"></i></span> <?php printf(__('Upgrading data to version %s', 'fieldmaster'), $plugin_version); ?>');
