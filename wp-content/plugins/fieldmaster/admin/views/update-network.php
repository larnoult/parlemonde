...
<div id="fields-upgrade-wrap" class="wrap">
	...
	<table class="wp-list-table widefat">
		...
		<thead>
			<tr>
				...
				<th class="manage-column" scope="col" style="width:33%;"><label for="sites-select-all"><?php _e("Site", 'fields'); ?></label></th>
				<th><?php _e("Description", 'fields'); ?></th>
			</tr>
		</thead>
		...
		<tr>
			...
			<th class="manage-column" scope="col"><label for="sites-select-all-2"><?php _e("Site", 'fields'); ?></label></th>
			<th><?php _e("Description", 'fields'); ?></th>
		</tr>
		</tfoot>
		...
		<?php foreach( $sites as $i => $site ): ?>
			...
			<tr<?php if( $i % 2 == 0 ): ?> class="alternate"<?php endif; ?>>
				...
				<td>
				<?php if( $site['updates'] ): ?>
					<span class="response"><?php printf(__('Site requires database upgrade from %s to %s', 'fields'), $site['fieldmaster_version'], $plugin_version); ?></span>
				<?php else: ?>
					<?php _e("Site is up to date", 'fields'); ?>
				<?php endif; ?>
				</td>
			</tr>
	...
	<p class="show-on-complete"><?php _e('Database Upgrade complete', 'fields'); ?>. <a href="<?php echo network_admin_url(); ?>"><?php _e("Return to network dashboard",'fields'); ?></a>.</p>
		...
		var upgrader = {
			...
			upgrade: function(){
				...
				var $input = this.$inputs.eq( this.i ),
					$tr = $input.closest('tr'),
					text = '<?php _e('Upgrade complete', 'fields'); ?>';
				
				
				// add loading
				$tr.find('.response').html('<i class="fields-loading"></i></span> <?php _e('Upgrading data to', 'fields'); ?> <?php echo $plugin_version; ?>');
