...
if( ! class_exists('fieldmaster_updates') ) :

class fieldmaster_updates {
	...
	*  request
	*
	*  This function will make a request to the FieldMaster update server
	*
	*  @type	function
	*  @date	8/4/17
	*  @since	5.5.10
	...
	*/
	...
	function request( $query = 'index.php', $body = array() ) {
		...
		// log
		//fieldmaster_log('fieldmaster connect: '. $url);
	...
	*/
	...
	function get_plugin_info( $id = '' ) {
		...
		// var
		$transient_name = 'fieldmaster_plugin_info_'.$id;
	...
	*
	*  This function will connect to the FieldMaster website and find update information
	*
	*  @type	function
	*  @date	16/01/2014
	*  @since	5.0.0
...
/*
	function modify_plugin_update_message( $plugin_data, $response ) {
		...
		if( isset($response->notice) ) {
			...
			echo '<div class="fieldmaster-plugin-upgrade-notice">' . $response->notice . '</div>';
...
*  fieldmaster_updates
*
*  The main function responsible for returning the one true fieldmaster_updates instance to functions everywhere.
*  Use this function like you would a global variable, except without needing to declare the global.
*
*  Example: <?php $fieldmaster_updates = fieldmaster_updates(); ?>
*
*  @type	function
*  @date	9/4/17
*  @since	5.5.12
...
function fieldmaster_updates() {

	global $fieldmaster_updates;

	if( !isset($fieldmaster_updates) ) {

		$fieldmaster_updates = new fieldmaster_updates();

	}

	return $fieldmaster_updates;

}


/*
*  fieldmaster_register_plugin_update
*
*  alias of fieldmaster_updates()->add_plugin()
*
*  @type	function
*  @date	12/4/17
*  @since	5.5.10
...
function fieldmaster_register_plugin_update( $plugin ) {

	fieldmaster_updates()->add_plugin( $plugin );
