...
if( ! class_exists('fieldmaster_local') ) :

class fieldmaster_local {
	...
	*/
	...
	function __construct() {
		...
		// actions
		add_action('fieldmaster/include_fields', 	array($this, 'fieldmaster_include_fields'), 5, 0);
		
		
		// filters
		add_filter('fieldmaster/get_field_groups',	array($this, 'fieldmaster_get_field_groups'), 20, 1);
	...
	*/
	...
	function is_enabled() {
		...
		if( !fieldmaster_get_setting('local') ) return false;
		
		
		// return
		return fieldmaster_is_filter_enabled('local');
	...
	*  is_ready
	*
	*  This function will return true when FieldMaster has included all field types and is ready to import
	...
	*/
	...
	function is_ready() {
		...
		return did_action('fieldmaster/include_fields');
		
	}
	
	
	/*
	*  fieldmaster_include_fields
	*
	...
	*/
	...
	function fieldmaster_include_fields() {
		...
		if( !empty($this->temp_fields) ) {
			...
			// prepare
			$this->temp_fields = fieldmaster_prepare_fields_for_import( $this->temp_fields );
	...
	*/
	...
	function get_field_groups() {
		...
		foreach( array_keys($this->groups) as $field_group_key ) {
			...
			$field_groups[] = fieldmaster_get_field_group( $field_group_key );
	...
	*/
	...
	function get_fields( $parent_key = '' ) {
		...
		foreach( $this->parents[ $parent_key ] as $field_key ) {
			...
			$fields[] = fieldmaster_get_field( $field_key );
	...
	*
	*  This function will override and add field groups to the `fieldmaster_get_field_groups()` results
	*
	*  @type	filter (fieldmaster/get_field_groups)
	*  @date	5/12/2013
	*  @since	5.0.0
	*
	...
	*/
	...
	function fieldmaster_get_field_groups( $field_groups ) {
...
// initialize
fieldmaster()->local = new fieldmaster_local();
...
*  Helpers
*
*  alias of fieldmaster()->local->functions
*
*  @type	function
*  @date	11/06/2014
*  @since	5.0.0
...
function fieldmaster_local() {
	
	return fieldmaster()->local;
	
}

function fieldmaster_disable_local() {
	
	fieldmaster_disable_filter('local');
	
}

function fieldmaster_enable_local() {
	
	fieldmaster_enable_filter('local');
	
}

function fieldmaster_reset_local() {
	
	return fieldmaster_local()->reset();
	
}


// field group
function fieldmaster_add_local_field_group( $field_group ) {
	
	return fieldmaster_local()->maybe_add_field_group( $field_group );
	
}

function fieldmaster_remove_local_field_group( $key = '' ) {
	
	return false;
	
}

function fieldmaster_is_local_field_group( $key = '' ) {
	
	return fieldmaster_local()->is_field_group( $key );
	
}

function fieldmaster_get_local_field_group( $key = '' ) {
	
	return fieldmaster_local()->get_field_group( $key );
	
}


// field groups
function fieldmaster_count_local_field_groups() {
	
	return fieldmaster_local()->count_field_groups();
	
}

function fieldmaster_have_local_field_groups() {
	
	return fieldmaster_local()->have_field_groups();
	
}

function fieldmaster_get_local_field_groups() {
	
	return fieldmaster_local()->get_field_groups();
	
}


// field
function fieldmaster_add_local_field( $field ) {
	
	return fieldmaster_local()->maybe_add_field( $field );
	
}

function fieldmaster_remove_local_field( $key = '' ) {
	
	return fieldmaster_local()->remove_field( $key );
	
}

function fieldmaster_is_local_field( $key = '' ) {
	
	return fieldmaster_local()->is_field( $key );
	
}

function fieldmaster_is_local_field_key( $key = '' ) {
	
	return fieldmaster_local()->is_field_key( $key );
	
}

function fieldmaster_is_local_field_name( $name = '' ) {
	
	return fieldmaster_local()->is_field_name( $name );
	
}

function fieldmaster_get_local_field( $key = '' ) {
	
	return fieldmaster_local()->get_field( $key );
	
}


// fields
function fieldmaster_count_local_fields( $key = '' ) {
	
	return fieldmaster_local()->count_fields( $key );
	
}

function fieldmaster_have_local_fields( $key = '' ) {
	
	return fieldmaster_local()->have_fields( $key );
	
}

function fieldmaster_get_local_fields( $key = '' ) {
	
	return fieldmaster_local()->get_fields( $key );
	
}

function fieldmaster_remove_local_fields( $key = '' ) {
	
	return fieldmaster_local()->remove_fields( $key );
