...
/*
*  FieldMaster Clone Field Class
*
*  All the logic for this field type
*
*  @class 		fieldmaster_field_clone
*  @extends		fieldmaster_field
*  @package		FieldMaster
*  @subpackage	Fields
*/

if( ! class_exists('fieldmaster_field_clone') ) :

class fieldmaster_field_clone extends fieldmaster_field {
	...
	*/
	...
	function __construct() {
		...
		// vars
		$this->name = 'clone';
		$this->label = _x('Clone', 'noun', 'fieldmaster');
		...
		// register filter
		fieldmaster_enable_filter('clone');
		
		
		// ajax
		add_action('wp_ajax_fieldmaster/fields/clone/query', array($this, 'ajax_query'));
		
		
		// filters
		add_filter('fieldmaster/get_fields', 		array($this, 'fieldmaster_get_fields'), 5, 2);
		add_filter('fieldmaster/prepare_field',		array($this, 'fieldmaster_prepare_field'), 10, 1);
		add_filter('fieldmaster/clone_field',		array($this, 'fieldmaster_clone_field'), 10, 2);
	...
	*  is_enabled
	*
	*  This function will return true if fieldmaster_local functionality is enabled
	*
	*  @type	function
	*  @date	14/07/2016
	*  @since	5.4.0
	...
	*/
	...
	function is_enabled() {
		...
		return fieldmaster_is_filter_enabled('clone');
	...
	*
	*  This function will hook into the 'fieldmaster/get_fields' filter and inject/replace seamless clones fields
	*
	*  @type	function
	*  @date	17/06/2016
	*  @since	5.3.8
	...
	*/
	...
	function fieldmaster_get_fields( $fields, $parent ) {
	...
	*/
	...
	function get_cloned_fields( $field ) {
		...
		foreach( $field['clone'] as $selector ) {
			...
			// field group
			if( fieldmaster_is_field_group_key($selector) ) {
				
				// vars
				$field_group = fieldmaster_get_field_group($selector);
				$field_group_fields = fieldmaster_get_fields($field_group);
			...
			// field
			} elseif( fieldmaster_is_field_key($selector) ) {
				
				// append
				$fields[] = fieldmaster_get_field($selector);
		...
		// loop
		// run fieldmaster_clone_field() on each cloned field to modify name, key, etc
		foreach( array_keys($fields) as $i ) {
			
			$fields[ $i ] = fieldmaster_clone_field( $fields[ $i ], $field );
	...
	*  Important to run the fieldmaster_clone_field function on sub fields to pass on settings such as 'parent_layout' 
	*
	*  @type	function
	*  @date	28/06/2016
	*  @since	5.3.8
	...
	*/
	...
	function fieldmaster_clone_field( $field, $clone_field ) {
		...
		// - the original key will later be restored by fieldmaster/prepare_field allowing conditional logic JS to work
		...
		if( $clone_field['prefix_name'] ) {
			...
			//fieldmaster_log('== fieldmaster_clone_field ==');
			//fieldmaster_log('- clone name', $clone_field['name']);
			//fieldmaster_log('- clone _name', $clone_field['_name']);
			...
			//fieldmaster_log('- field name:', $name, '=>', $field['name']);
			...
			//fieldmaster_log('- field _name:', $_name, '=>', $field['_name']);
		...
		if( $field['type'] == 'clone' ) {
			...
			$field = $this->fieldmaster_clone_clone_field( $field, $clone_field );
	...
	*/
	...
	function fieldmaster_clone_clone_field( $field, $clone_field ) {
		...
		foreach( array_keys($field['sub_fields']) as $i ) {
			...
			// clone
			$sub_field = fieldmaster_clone_field( $sub_field, $clone_field );
	...
	*/
	...
	function prepare_field_for_db( $field ) {
		...
		//fieldmaster_log('== prepare_field_for_db ==');
		//fieldmaster_log('- clone name:', $field['name']);
		//fieldmaster_log('- clone _name:', $field['_name']);
		...
		foreach( array_keys($field['sub_fields']) as $i ) {
			...
			//fieldmaster_log('- field name:', $name, '=>', $sub_field['name']);
	...
	*/
	...
	function format_value( $value, $post_id, $field ) {
		...
		foreach( array_keys($field['sub_fields']) as $i ) {
			...
			// extract value
			$sub_value = fieldmaster_extract_var( $value, $sub_field['key'] );
	...
	*/
	...
	function update_value( $value, $post_id, $field ) {
		...
		// bail early if no value
		if( !fieldmaster_is_array($value) ) return null;
		...
		foreach( array_keys($field['sub_fields']) as $i ) {
			...
			$sub_field = $this->fieldmaster_prepare_field( $sub_field );
			
			
			// update value
			fieldmaster_update_value( $v, $post_id, $sub_field );
	...
	*/
	...
	function render_field_block( $field ) {
		...
		// html
		echo '<div class="fieldmaster-clone-fields fm-fields -'.$label_placement.'">';
			
		foreach( $field['sub_fields'] as $sub_field ) {
			
			fieldmaster_render_field_wrap( $sub_field );
...
<table class="fieldmaster-table">
	<thead>
		...
		<?php foreach( $field['sub_fields'] as $sub_field ): 
			...
			?>
			<th <?php fieldmaster_esc_attr_e( $atts ); ?>>
	...
	<tbody>
		<tr class="fieldmaster-row">
		<?php 
		
		foreach( $field['sub_fields'] as $sub_field ) {
			
			fieldmaster_render_field_wrap( $sub_field, 'td' );
...
<?php
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		fieldmaster_render_field_setting( $field, array(
			'label'			=> __('Fields', 'fieldmaster'),
			'instructions'	=> __('Select one or more fields you wish to clone','fieldmaster'),
			'type'			=> 'select',
			'name'			=> 'clone',
			'multiple' 		=> 1,
			'allow_null' 	=> 1,
			...
			'ui'			=> 1,
			'ajax'			=> 1,
			'ajax_action'	=> 'fieldmaster/fields/clone/query',
			'placeholder'	=> '',
		));
		
		fieldmaster_disable_filter('local');
		
		
		// display
		fieldmaster_render_field_setting( $field, array(
			'label'			=> __('Display','fieldmaster'),
			'instructions'	=> __('Specify the style used to render the clone field', 'fieldmaster'),
			'type'			=> 'select',
			'name'			=> 'display',
			'class'			=> 'setting-display',
			'choices'		=> array(
				'group'			=> __('Group (displays selected fields in a group within this field)','fieldmaster'),
				'seamless'		=> __('Seamless (replaces this field with selected fields)','fieldmaster'),
			),
		));
		...
		fieldmaster_render_field_setting( $field, array(
			...
			'instructions'	=> __('Specify the style used to render the selected fields', 'fieldmaster'),
			'type'			=> 'radio',
			'name'			=> 'layout',
			'layout'		=> 'horizontal',
			'choices'		=> array(
		...
		// prefix_label
		$instructions = __('Labels will be displayed as %s', 'fieldmaster');
		$instructions = sprintf($instructions, '<code class="prefix-label-code-1"></code>');
		fieldmaster_render_field_setting( $field, array(
			'label'			=> __('Prefix Field Labels','fieldmaster'),
		...
		// prefix_name
		$instructions = __('Values will be saved as %s', 'fieldmaster');
		$instructions = sprintf($instructions, '<code class="prefix-name-code-1"></code>');
		fieldmaster_render_field_setting( $field, array(
			'label'			=> __('Prefix Field Names','fieldmaster'),
			'message'	=> $instructions,
			//'instructions_placement'	=> 'field',
			'name'			=> 'prefix_name',
	...
	*/
	...
	function get_clone_setting_choice( $selector = '' ) {
		...
		if( fieldmaster_is_field_key($selector) ) {
			...
			return $this->get_clone_setting_field_choice( fieldmaster_get_field($selector) );
		...
		if( fieldmaster_is_field_group_key($selector) ) {
			...
			return $this->get_clone_setting_group_choice( fieldmaster_get_field_group($selector) );
	...
	*/
	...
	function get_clone_setting_field_choice( $field ) {
		...
		if( !$field ) return __('Unknown field', 'fieldmaster');
		
		
		// title
		$title = $field['label'] ? $field['label'] : __('(no title)', 'fieldmaster');
		...
		$ancestors = isset($field['ancestors']) ? $field['ancestors'] : count(fieldmaster_get_field_ancestors($field));
	...
	*/
	...
	function get_clone_setting_group_choice( $field_group ) {
		...
		if( !$field_group ) return __('Unknown field group', 'fieldmaster');
		
		
		// return
		return sprintf( __('All fields from %s field group', 'fieldmaster'), $field_group['title'] );
	...
	*/
	...
	function ajax_query() {
		...
		fieldmaster_disable_filter('clone');
		...
		foreach( array_keys($field_groups) as $j ) {
			...
			$field_group = fieldmaster_extract_var($field_groups, $j);
	...
	*/
	...
	function validate_value( $valid, $value, $field, $input ){
		...
		foreach( array_keys($field['sub_fields']) as $i ) {
			...
			// validate
			fieldmaster_validate_value( $value[ $k ], $sub_field, "{$input}[{$k}]" );
...
// initialize
fieldmaster_register_field_type( new fieldmaster_field_clone() );
