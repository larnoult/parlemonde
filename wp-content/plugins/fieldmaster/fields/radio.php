...
class fieldmaster_field_radio extends fieldmaster_field {
	...
	*/
	...
	function __construct() {
		...
		// vars
		$this->name = 'radio';
		$this->label = __("Radio Button",'fieldmaster');
	...
	*/
	...
	function render_field( $field ) {
		...
		// append to class
		$ul['class'] .= ' ' . ($field['layout'] == 'horizontal' ? 'fieldmaster-hl' : 'fieldmaster-bl');
		...
		if( $field['other_choice'] ) {
			...
			// append other choice
			$field['choices']['other'] .= '</label><input type="text" ' . fieldmaster_esc_attr($input) . ' /><label>';
		...
		// hiden input
		$e .= fieldmaster_get_hidden_input( array('name' => $field['name']) );
		
		
		// open
		$e .= '<ul ' . fieldmaster_esc_attr($ul) . '>';
		...
		foreach( $field['choices'] as $value => $label ) {
			...
			// append
			$e .= '<li><label' . $class . '><input ' . fieldmaster_esc_attr( $atts ) . '/>' . $label . '</label></li>';
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		// other_choice
		fieldmaster_render_field_setting( $field, array(
			'label'			=> __('Other','fieldmaster'),
			'instructions'	=> '',
			'name'			=> 'other_choice',
			'type'			=> 'true_false',
			'ui'			=> 1,
			'message'		=> __("Add 'other' choice to allow for custom values", 'fieldmaster'),
		));
		
		
		// save_other_choice
		fieldmaster_render_field_setting( $field, array(
			'label'			=> __('Save Other','fieldmaster'),
			'instructions'	=> '',
			'name'			=> 'save_other_choice',
			'type'			=> 'true_false',
			'ui'			=> 1,
			'message'		=> __("Save 'other' values to the field's choices", 'fieldmaster')
		));
...
// initialize
fieldmaster_register_field_type( new fieldmaster_field_radio() );
