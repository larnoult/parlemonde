(function($){
	...
	fieldmaster.field_group = fieldmaster.model.extend({
		...
		events: {
			...
			'mouseenter .fm-field-list': 	'sortable'
		},
		...
		*/
		...
		init: function(){
			...
			// $el
			this.$fields = $('#fm-field-group-fields');
			this.$locations = $('#fm-field-group-locations');
			this.$options = $('#fm-field-group-options');
			
			
			// disable validation
			fieldmaster.validation.active = 0;
		...
		*/
		...
		sortable: function( e ){
			...
			// sortable
			e.$el.sortable({
				handle: '.fieldmaster-sortable-handle',
				connectWith: '.fm-field-list',
				update: function(event, ui){
					...
					// actions
					fieldmaster.do_action('sortstop', $el);
		...
		*/
		...
		render_fields: function(){
			...
			$('.fm-field-list').each(function(){
				
				// vars
				var $fields = $(this).children('.fm-field-object');
				...
				$fields.each(function( i ){
					...
					// update icon number
					$(this).children('.handle').find('.fieldmaster-icon').html( i+1 );
		...
		*/
		...
		update_field_meta: function( $el, name, value ){
			...
			if( !$input.exists() ) {
				...
				// replcae
				html = fieldmaster.str_replace('ID', name, html);
		...
		*/
		...
		save_field: function( $el, type ){
			...
			fieldmaster.do_action('save_field', $el, type);
		...
		*/
		...
		submit: function( e ){
			...
			if( !$title.val() ) {
				...
				// unlock form
				fieldmaster.validation.toggle( e.$el, 'unlock' );
				
				
				// alert
				alert( fieldmaster._e('title_is_required') );
		...
		*/
		...
		trash: function( e ){
			...
			var result = confirm( fieldmaster._e('move_to_trash') );
		...
		*/
		...
		render_field: function( $el ){
			...
			// update required
			$handle.find('.li-field-label .fieldmaster-required').remove();
			
			if( required ) {
				
				$handle.find('.li-field-label strong').append('<span class="fieldmaster-required">*</span>');
			...
			// action for 3rd party customization
			fieldmaster.do_action('render_field_handle', $el, $handle);
		...
		*/
		...
		open_field: function( $el ){
			...
			// action for 3rd party customization
			fieldmaster.do_action('open_field', $el);
		...
		*/
		...
		close_field: function( $el ){
			...
			// action for 3rd party customization
			fieldmaster.do_action('close_field', $el);
		...
		*/
		...
		wipe_field: function( $el ){
			...
			// action for 3rd party customization
			fieldmaster.do_action('wipe_field', $el);
		...
		*/
		...
		add_field: function( $fields ){
			...
			// trigger append
			fieldmaster.do_action('append', $el);
			...
			// action for 3rd party customization
			fieldmaster.do_action('add_field', $el);
		...
		*/
		...
		move_field: function( $field ){
			...
			var ajax_data = fieldmaster.prepare_for_ajax({
				action:		'fieldmaster/field_group/move_field',
			...
			if( !ajax_data.field_id ) {
				...
				$field.find('.fm-field-object').each(function(){
			...
			if( warning ) {
				...
				alert( fieldmaster._e('move_field_warning') );
				return;
			...
			fieldmaster.open_popup({
				title	: fieldmaster._e('move_field'),
				loading	: true,
				height	: 145
			});
		...
		*/
		...
		move_field_confirm: function( $field, html ){
			...
			// submit form
			$('#fieldmaster-move-field-form').on('submit', function(){
		...
		*/
		...
		delete_field: function( $el, animation ){
			...
			// action for 3rd party customization
			fieldmaster.do_action('delete_field', $el);
		...
		*/
		...
		remove_field: function( $el ){
			...
			// vars
			var $field_list	= $el.closest('.fm-field-list');
			...
			if( !$field_list.children('.fm-field-object').length ) {
			...
			$el.parent('.temp-field-wrap').animate({ height : end_height }, 250, function(){
				...
				// action for 3rd party customization 
				fieldmaster.do_action('remove', $(this));
		...
		*/
		...
		change_field_type: function( $select ){
			...
			var $tbody		= $select.closest('tbody'),
				$el			= $tbody.closest('.fm-field-object'),
				$parent		= $el.parent().closest('.fm-field-object'),
			...
			// update class
			$el.removeClass( 'fm-field-object-' + fieldmaster.str_replace('_', '-', old_type) );
			$el.addClass( 'fm-field-object-' + fieldmaster.str_replace('_', '-', new_type) );
			...
			// get settings
			var $settings = $tbody.children('.fm-field[data-setting="' + old_type + '"]'),
				html = '';
			...
			fieldmaster.update( key + '_settings_' + old_type, html );
			...
			html = fieldmaster.get( key + '_settings_' + new_type );
			
			if( html ) {
				
				// append settings
				$tbody.children('.fm-field[data-name="conditional_logic"]').before( html );
				
				
				// remove field settings html
				fieldmaster.update( key + '_settings_' + new_type, '' );
				
				
				// trigger event
				fieldmaster.do_action('change_field_type', $el);
			...
			// add loading
			var $tr = $('<tr class="fm-field"><td class="fieldmaster-label"></td><td class="fieldmaster-input"><div class="fieldmaster-loading"></div></td></tr>');
			
			
			// add $tr
			$tbody.children('.fm-field[data-name="conditional_logic"]').before( $tr );
			
			
			var ajax_data = {
				action		: 'fieldmaster/field_group/render_field_settings',
				nonce		: fieldmaster.o.nonce,
				parent		: fieldmaster.o.post_id,
				field_group	: fieldmaster.o.post_id,
			...
			var xhr = $.ajax({
				...
				success: function( html ){
					...
					// trigger event
					fieldmaster.do_action('append', $new_tr);
					fieldmaster.do_action('change_field_type', $el);
		...
		*/
		...
		change_field_label: function( $el ) {
			...
			// action for 3rd party customization
			fieldmaster.do_action('change_field_label', $el);
		...
		*/
		...
		change_field_name: function( $el ) {
			...
			if( $name.val().substr(0, 6) === 'field_' ) {
				...
				alert( fieldmaster._e('field_name_start') );
			...
			// action for 3rd party customization
			fieldmaster.do_action('change_field_name', $el);
	...
	*/
	...
	fieldmaster.field_group.field = fieldmaster.model.extend({
		...
		events: {
			...
			'change .fm-field-object input':		'save',
			'change .fm-field-object textarea':	'save',
			'change .fm-field-object select':		'save'
		},
		
		event: function( e ){
			
			// append $field
			e.$field = e.$el.closest('.fm-field-object');
		...
		*/
		...
		edit: function( e ){
			...
			fieldmaster.field_group.edit_field( e.$field );
				
		},
		
		duplicate: function( e ){
			
			fieldmaster.field_group.duplicate_field( e.$field );
				
		},
		
		move: function( e ){
			
			fieldmaster.field_group.move_field( e.$field );
				
		},
		
		delete: function( e ){
			
			fieldmaster.field_group.delete_field( e.$field );
				
		},
		
		add: function( e ){
			
			var $list = e.$el.closest('.fm-field-list-wrap').children('.fm-field-list');
			
			fieldmaster.field_group.add_field( $list );
				
		},
		
		change_type: function( e ){
			
			fieldmaster.field_group.change_field_type( e.$el );
			
		},
		
		change_label: function( e ){
			
			fieldmaster.field_group.change_field_label( e.$field );
			
		},
		
		change_name: function( e ){
			
			fieldmaster.field_group.change_field_name( e.$field );
			
		},
		
		render: function( e ){
			
			fieldmaster.field_group.render_field( e.$field );
				
		},
		
		save: function( e ){
			
			fieldmaster.field_group.save_field( e.$field );
	...
	*/
	...
	fieldmaster.field_group.conditional_logic = fieldmaster.model.extend({
		...
		*/
		...
		render_fields: function(){
			...
			$('.fm-field-object.open').each(function(){
		...
		*/
		...
		render_field: function( $field ){
			...
			var key			= $field.attr('data-key'),
				$ancestors	= $field.parents('.fm-field-list'),
				$tr			= $field.find('.fm-field[data-name="conditional_logic"]:last');
			...
			$.each( $ancestors, function( i ){
				...
				// vars
				var group = (i == 0) ? fieldmaster._e('sibling_fields') : fieldmaster._e('parent_fields');
				
				
				// loop over fields
				$(this).children('.fm-field-object').each(function(){
			...
			if( !choices.length ) {
				...
				choices.push({
					value: '',
					label: fieldmaster._e('no_fields')
				});
		...
		*/
		...
		render_rule: function( $tr, triggers ) {
			...
			if( triggers ) {
				...
				fieldmaster.render_select( $trigger, triggers );
				
			}
			
			
			// vars
			var $field		= $('.fm-field-object[data-key="' + $trigger.val() + '"]'),
			...
			// select				
				...
				// vars
				var lines = $field.find('.fm-field[data-name="choices"] textarea').val().split("\n");	
				...
				// allow null
				var $allow_null = $field.find('.fm-field[data-name="allow_null"]');
			...
			// update select
			fieldmaster.render_select( $value, choices );
		...
		*/
		...
		change_toggle: function( e ){
			...
			var $input = e.$el,
				...
				$td = $input.closest('.fieldmaster-input');
		...
		*/
		...
		add_group: function( e ){
			...
			// duplicate
			$group2 = fieldmaster.duplicate( $group );
			
			
			// update h4
			$group2.find('h4').text( fieldmaster._e('or') );
	...
	*/
	...
	fieldmaster.field_group.locations = fieldmaster.model.extend({
		...
		*/
		...
		change_rule: function( e ){
			...
			// add loading gif
			var $div = $('<div class="fieldmaster-loading"></div>');
			...
			$.ajax({
				...
				data: fieldmaster.prepare_for_ajax({
					'action':	'fieldmaster/field_group/render_location_value',
					'rule_id':	rule_id,
					'group_id':	group_id,
					'param':	$select.val(),
					'value':	''
	...
	*/
	...
	fieldmaster.field_group.field_object = fieldmaster.model.extend({
		...
		_add_event: function( name, callback ) {
			...
			var model = this,
				...
				context = fieldmaster.field_group.get_selector(model.type);
		...
		setting: function( name ) {
			...
			return this.$settings.find('> .fm-field-setting-' + name);
	...
	*/
	...
	fieldmaster.field_group.field_objects = fieldmaster.model.extend({
		...
		_save_field: function( $el ){
			...
			fieldmaster.do_action('save_field/type=' + $el.data('type'), $el);
			
		},
		
		_open_field: function( $el ){
			
			fieldmaster.do_action('open_field/type=' + $el.data('type'), $el);
			fieldmaster.do_action('render_field_settings', $el);
			
		},
		
		_close_field: function( $el ){
			
			fieldmaster.do_action('close_field/type=' + $el.data('type'), $el);
			
		},
		
		_wipe_field: function( $el ){
			
			fieldmaster.do_action('wipe_field/type=' + $el.data('type'), $el);
			
		},
		
		_add_field: function( $el ){
			
			fieldmaster.do_action('add_field/type=' + $el.data('type'), $el);
			
		},
		
		_duplicate_field: function( $el ){
			
			fieldmaster.do_action('duplicate_field/type=' + $el.data('type'), $el);
			
		},
		
		_delete_field: function( $el ){
			
			fieldmaster.do_action('delete_field/type=' + $el.data('type'), $el);
			
		},
		
		_change_field_type: function( $el ){
			
			fieldmaster.do_action('change_field_type/type=' + $el.data('type'), $el);
			fieldmaster.do_action('render_field_settings', $el);
		},
		
		_change_field_label: function( $el ){
			
			fieldmaster.do_action('change_field_label/type=' + $el.data('type'), $el);
			
		},
		
		_change_field_name: function( $el ){
			
			fieldmaster.do_action('change_field_name/type=' + $el.data('type'), $el);
			
		},
		
		_render_field_settings: function( $el ){
			
			fieldmaster.do_action('render_field_settings/type=' + $el.data('type'), $el);
	...
	*/
	...
	fieldmaster.field_group.append = fieldmaster.model.extend({
		...
		render: function( $el ){
			...
			// vars
			var $wrap = $sibling.children('.fieldmaster-input'),
				$ul = $wrap.children('.fieldmaster-hl');
			
			
			// append ul if doesn't exist
			if( !$ul.exists() ) {
				
				$wrap.wrapInner('<ul class="fieldmaster-hl"><li></li></ul>');
			...
			// create $li
			var $li = $('<li></li>').append( $el.children('.fieldmaster-input').children() );
		...
		_render_field_settings: function( $el ){
			...
			// loop
			$el.find('.fm-field[data-append]').each(function(){
	...
	*/
	...
	var fieldmaster_settings_select = fieldmaster.field_group.field_object.extend({
		...
		events: {
			'change .fm-field-setting-ui input': 'render'
		},
	...
	*/
	...
	var fieldmaster_settings_radio = fieldmaster.field_group.field_object.extend({
		...
		events: {
			'change .fm-field-setting-other_choice input': 'render'
		},
	...
	*/
	...
	var fieldmaster_settings_true_false = fieldmaster.field_group.field_object.extend({
	...
	*/
	...
	var fieldmaster_settings_date_picker = fieldmaster.field_group.field_object.extend({
		...
		events: {
			'change .fm-field-setting-display_format input':	'render',
			'change .fm-field-setting-return_format input':	'render'
		},
	...
	*/
	...
	var fieldmaster_settings_date_time_picker = fieldmaster_settings_date_picker.extend({
	...
	*/
	...
	var fieldmaster_settings_tab = fieldmaster.field_group.field_object.extend({
	...
	*/
	...
	var fieldmaster_settings_message = fieldmaster_settings_tab.extend({
	...
	*/
	...
	fieldmaster.field_group.screen = fieldmaster.model.extend({
		...
		events: {
			'click #fm-field-key-hide': 'toggle'
		},
		...
		ready: function(){
			...
			// vars
			var $el = $('#adv-settings'),
				$append = $el.find('#fieldmaster-append-show-on-screen');
		...
		toggle: function( e ){
			...
			// update user setting
			fieldmaster.update_user_setting('show_field_keys', val);
		...
		render: function(){
			...
			// vars
			var options = fieldmaster.serialize_form( $('#adv-settings') );
			...
			// toggle class
			var $fields = fieldmaster.field_group.$fields;
	...
	*/
	...
	fieldmaster.field_group.sub_fields = fieldmaster.model.extend({
    	...
    	*/
    	...
    	update_field_parent: function( $el ){
	    	...
	    	if( !$el.hasClass('fm-field-object') ) return;
	    	
	    	
	    	// vars
	    	var $parent = $el.parent().closest('.fm-field-object'),
		    	val = fieldmaster.get('post_id');
			...
			if( $parent.exists() ) {
				...
				// set as parent ID
				val = fieldmaster.field_group.get_field_meta( $parent, 'ID' );
				
				
				// if parent is new, no ID exists
				if( !val ) {
					
					val = fieldmaster.field_group.get_field_meta( $parent, 'key' );
			...
			// update parent
			fieldmaster.field_group.update_field_meta( $el, 'parent', val );
	    	
	    	
	    	// action for 3rd party customization
			fieldmaster.do_action('update_field_parent', $el, $parent);
    	...
    	*/
    	...
    	duplicate_field: function( $el ) {
	    	...
	    	// vars
			var $fields = $el.find('.fm-field-object');
	    	...
	    	$fields.each(function(){
		    	...
		    	// vars
		    	var $parent = $(this).parent().closest('.fm-field-object'),
		    		key = fieldmaster.field_group.get_field_meta( $parent, 'key');
		    		
		    	
		    	// wipe field
		    	fieldmaster.field_group.wipe_field( $(this) );
		    	
		    	
		    	// update parent
		    	fieldmaster.field_group.update_field_meta( $(this), 'parent', key );
		    	
		    	
		    	// save field
		    	fieldmaster.field_group.save_field( $(this) );
