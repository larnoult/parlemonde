(function($){
	...
	fieldmaster.fields.repeater = fieldmaster.field.extend({
		...
		focus: function(){
			...
			// vars
			this.$el = this.$field.find('.fieldmaster-repeater:first');
			this.$input = this.$field.find('input:first');
			this.$table = this.$field.find('table:first');
			this.$tbody = this.$table.children('tbody');
			this.$clone = this.$tbody.children('tr.fieldmaster-clone');
		...
		initialize: function(){
			...
			// disable clone
			fieldmaster.disable_form( this.$clone, 'repeater' );
		...
		show: function(){
			...
			this.$tbody.find('.fm-field:visible').each(function(){
		...
		render: function(){
			...
			if( this.o.max > 0 && this.count() >= this.o.max ) {
				...
				this.$el.find('> .fieldmaster-actions .button').addClass('disabled');
				
			} else {
				
				this.$el.find('> .fieldmaster-actions .button').removeClass('disabled');
		...
		add: function( $tr ){
			...
			if( this.o.max > 0 && this.count() >= this.o.max ) {
				...
				alert( fieldmaster._e('repeater','max').replace('{max}', this.o.max) );
				return false;
			...
			// duplicate
			$el = fieldmaster.duplicate( this.$clone );
			
						
			// remove clone class
			$el.removeClass('fieldmaster-clone');
			
			
			// enable 
			fieldmaster.enable_form( $el, 'repeater' );
		...
		remove: function( $tr ){
			...
			if( this.count() <= this.o.min ) {
				...
				alert( fieldmaster._e('repeater','min').replace('{min}', this.o.min) );
				return false;
			}
			
			
			// action for 3rd party customization
			fieldmaster.do_action('remove', $tr);
			
			
			// animate out tr
			fieldmaster.remove_tr( $tr, function(){
				...
				fieldmaster.do_action('refresh', self.$field);
		...
		sync: function(){
			...
			// update
			fieldmaster.update_user_setting( name, collapsed.join(',') );	
		...
		*/
		...
		_add: function( e ){ //console.log('_add');
			...
			// row add
			if( e.$el.hasClass('fieldmaster-icon') ) {
			
				$row = e.$el.closest('.fieldmaster-row');
		...
		_remove: function( e ){ //console.log('_remove');
			...
			this.remove( e.$el.closest('.fieldmaster-row') );
		...
		_collapse: function( e ){ //console.log('_collapse');
			...
			// vars
			var $tr = e.$el.closest('.fieldmaster-row');
			...
			if( $tr.hasClass('-collapsed') ) {
				...
				fieldmaster.do_action('show', $tr, 'collapse');
				
			} else {
				
				$tr.addClass('-collapsed');
				
				fieldmaster.do_action('hide', $tr, 'collapse');
			...
			// refersh field (hide/show columns)
			fieldmaster.do_action('refresh', this.$field);
...
(function($){
	...
	fieldmaster.fields.flexible_content = fieldmaster.field.extend({
		...
		events: {
			...
			'click .fieldmaster-fc-layout-handle':				'_collapse',
			'click .fieldmaster-fc-popup a':					'_add',
			'blur .fieldmaster-fc-popup .focus':				'_close',
			'mouseenter .fieldmaster-fc-layout-handle': 		'_mouseenter'
		},
		
		focus: function(){
			
			// vars
			this.$el = this.$field.find('.fieldmaster-flexible-content:first');
		...
		initialize: function(){
			...
			// disable clone
			fieldmaster.disable_form( this.$clones, 'flexible_content' );
		...
		show: function(){
			...
			this.$values.find('.fm-field:visible').each(function(){
		...
		render: function(){
			...
			this.$values.children('.layout').each(function( i ){
				...
				$(this).find('> .fieldmaster-fc-layout-handle .fieldmaster-fc-layout-order').html( i+1 );
		...
		render_layout_title: function( $layout ){
			...
			var prefix = $input.attr('name').replace('[fieldmaster_fc_layout]', '');
			
			
			// ajax data
			var ajax_data = fieldmaster.prepare_for_ajax({
				action: 	'fieldmaster/fields/flexible_content/layout_title',
				field_key: 	this.$field.data('key'),
				i: 			$layout.index(),
				layout:		$input.val(),
				value:		fieldmaster.serialize( $layout, prefix )
			});
			...
			$.ajax({
				...
				success: function( html ){
					...
					// update html
					$layout.find('> .fieldmaster-fc-layout-handle').html( html );
		...
		validate_add: function( layout ){
			...
			if( max && count >= max ) {
				...
				var identifier	= ( max == 1 ) ? 'layout' : 'layouts',
					s 			= fieldmaster._e('flexible_content', 'max');
				
				
				// translate
				s = s.replace('{max}', max);
				s = s.replace('{identifier}', fieldmaster._e('flexible_content', identifier));
			...
			if( layout ) {
				...
				if( layout_max > 0 && layout_count >= layout_max ) {
					...
					var identifier	= ( layout_max == 1 ) ? 'layout' : 'layouts',
						s 			= fieldmaster._e('flexible_content', 'max_layout');
		...
		validate_remove: function( layout ){
			...
			if( min > 0 && count <= min ) {
				...
				var identifier	= ( min == 1 ) ? 'layout' : 'layouts',
					s 			= fieldmaster._e('flexible_content', 'min') + ', ' + fieldmaster._e('flexible_content', 'remove');
				
				
				// translate
				s = s.replace('{min}', min);
				s = s.replace('{identifier}', fieldmaster._e('flexible_content', identifier));
				s = s.replace('{layout}', fieldmaster._e('flexible_content', 'layout'));
			...
			if( layout ) {
				...
				if( layout_min > 0 && layout_count <= layout_min ) {
					...
					var identifier	= ( layout_min == 1 ) ? 'layout' : 'layouts',
						s 			= fieldmaster._e('flexible_content', 'min_layout') + ', ' + fieldmaster._e('flexible_content', 'remove');
		...
		add: function( layout, $before ){
			...
			// duplicate
			$el = fieldmaster.duplicate( $clone );
			
			
			// enable 
			fieldmaster.enable_form( $el, 'flexible_content' );
		...
		*/
		...
		_mouseenter: function( e ){ //console.log('_mouseenter');
			...
			this.$values.sortable({
				items: '> .layout',
				handle: '> .fieldmaster-fc-layout-handle',
				forceHelperSize: true,
				forcePlaceholderSize: true,
				scroll: true,
				start: function(event, ui) {
		...
		_open: function( e ){ //console.log('_open');
			...
			$popup.find('a').each(function(){
				...
				if( min ) {
					...
					var required	= min - count,
						s			= fieldmaster._e('flexible_content', 'required'),
			...
			// within layout?
			if( e.$el.closest('.fieldmaster-fc-layout-controlls').exists() ) {
		...
		_add: function( e ){ //console.log('_add');
			...
			// vars
			var $popup = e.$el.closest('.fieldmaster-fc-popup'),
				layout = e.$el.attr('data-layout'),
				$before = false;
			
			
			// move row
			if( $popup.closest('.fieldmaster-fc-layout-controlls').exists() ) {
		...
		_remove: function( e ){ //console.log('_remove');
			...
			// action for 3rd party customization
			fieldmaster.do_action('remove', $layout);
			
			
			// remove
			fieldmaster.remove_el( $layout, function(){
		...
		_collapse: function( e ){ //console.log('_collapse');
			...
			// action
			fieldmaster.do_action(action, $layout, 'collapse');
...
(function($){
	...
	fieldmaster.fields.gallery = fieldmaster.field.extend({
		...
		events: {
			'click .fieldmaster-gallery-attachment': 		'_select',
			'click .fieldmaster-gallery-add':				'_add',
			'click .fieldmaster-gallery-remove':			'_remove',
			'click .fieldmaster-gallery-close':				'_close',
			'change .fieldmaster-gallery-sort':				'_sort',
			'click .fieldmaster-gallery-edit':				'_edit',
			'click .fieldmaster-gallery-update': 			'_update',
			
			'change .fieldmaster-gallery-side input':		'_update',
			'change .fieldmaster-gallery-side textarea':	'_update',
			'change .fieldmaster-gallery-side select':		'_update'
		},
		...
		*/
		...
		focus: function(){
			...
			// el
			this.$el = this.$field.find('.fieldmaster-gallery:first');
			this.$main = this.$el.children('.fieldmaster-gallery-main');
			this.$side = this.$el.children('.fieldmaster-gallery-side');
			this.$attachments = this.$main.children('.fieldmaster-gallery-attachments');
		...
		*/
		...
		initialize: function(){
			...
			this.$attachments.unbind('sortable').sortable({
				...
				items					: '.fieldmaster-gallery-attachment',
				forceHelperSize			: true,
				forcePlaceholderSize	: true,
				scroll					: true,
			...
			this.$el.unbind('resizable').resizable({
				...
				stop: function(event, ui){
					...
					fieldmaster.update_user_setting('gallery_height', ui.size.height);
		...
		*/
		...
		render: function() {
			...
			// vars
			var $select = this.$main.find('.fieldmaster-gallery-sort'),
				$a = this.$main.find('.fieldmaster-gallery-add');
		...
		*/
		...
		open_sidebar: function(){
			...
			// hide bulk actions
			this.$main.find('.fieldmaster-gallery-sort').hide();
			...
			// animate
			this.$side.children('.fieldmaster-gallery-side-inner').css({ 'width' : width-1 });
		...
		*/
		...
		close_sidebar: function(){
			...
			// vars
			var $select = this.$el.find('.fieldmaster-gallery-sort');
			...
			this.$side.animate({ width: 0 }, 250, function(){
				...
				$(this).find('.fieldmaster-gallery-side-data').html('');
		...
		*/
		...
		get_attachments: function(){
			...
			return this.$attachments.children('.fieldmaster-gallery-attachment');
		...
		*/
		...
		get_attachment: function( s ){
			...
			// return
			return this.$attachments.children( '.fieldmaster-gallery-attachment'+s );
		...
		*/
		...
		render_attachment: function( data ){
			...
			// other (video)	
				...
				thumbnail = fieldmaster.maybe_get(data, 'thumb.src');
			...
			if( !thumbnail ) {
				...
				thumbnail = fieldmaster._e('media', 'default_icon');
		...
		_add: function( e ){
			...
			if( this.o.max > 0 && this.count() >= this.o.max ) {
				...
				fieldmaster.validation.add_warning( this.$field, fieldmaster._e('gallery', 'max'));
			...
			var frame = fieldmaster.media.popup({
				...
				title:		fieldmaster._e('gallery', 'select'),
				mode:		'select',
				type:		'',
		...
		*/
		...
		add_attachment: function( data, i ){
			...
			// html
			var html = [
			'<div class="fieldmaster-gallery-attachment fieldmaster-soh" data-id="' + data.id + '">',
				...
				'<div class="actions fieldmaster-soh-target">',
					'<a href="#" class="fieldmaster-icon -cancel dark fieldmaster-gallery-remove" data-id="' + data.id + '"></a>',
				'</div>',
			'</div>'].join('');
		...
		*/
		...
		prepare: function( attachment ) {
			...
			if( attachment.id ) {
				...
				data.url = fieldmaster.maybe_get(data, 'sizes.medium.url', data.url);
		...
		*/
		...
		fetch: function( id ){
			...
			var data = fieldmaster.prepare_for_ajax({
				action		: 'fieldmaster/fields/gallery/get_attachment',
				field_key	: this.$field.data('key'),
				id			: id
			});
		...
		fetch_success: function( html ){
			...
			// vars
			var $side = this.$side.find('.fieldmaster-gallery-side-data');
			...
			// remove fieldmaster form data
			$side.find('.compat-field-fieldmaster-form-data').remove();
			...
			// setup fields
			fieldmaster.do_action('append', $side);
		...
		*/
		...
		_sort: function( e ){
			...
			// vars
			var data = fieldmaster.prepare_for_ajax({
				action		: 'fieldmaster/fields/gallery/get_sort_order',
				field_key	: this.$field.data('key'),
				ids			: [],
				sort		: sort
			});
		...
		_sort_success: function( json ) {
			...
			// validate
			if( !fieldmaster.is_ajax_success(json) ) return;
		...
		*/
		...
		_update: function(){
			...
			// vars
			var $submit = this.$side.find('.fieldmaster-gallery-update'),
				$edit = this.$side.find('.fieldmaster-gallery-edit'),
				$form = this.$side.find('.fieldmaster-gallery-side-data'),
				id = $edit.data('id'),
				data = fieldmaster.serialize_form( $form );
			...
			$submit.before('<i class="fieldmaster-loading"></i>');
			
			
			// append AJAX action		
			data.action = 'fieldmaster/fields/gallery/update_attachment';
			
			
			// prepare for ajax
			fieldmaster.prepare_for_ajax(data);
			...
			$.ajax({
				...
				complete	: function( json ){
					...
					$submit.prev('.fieldmaster-loading').remove();
				...
				if( collection ) {
					...
					collection.each(function( item ){
						...
						if( self.get_attachment(item.id).exists() ) {
							...
							$li.addClass('fieldmaster-selected');
	...
	*/
	...
	var fieldmaster_gallery_manager = fieldmaster.model.extend({
		...
		validation_begin: function(){
			...
			$('.fieldmaster-gallery-side-data').each(function(){
				
				fieldmaster.disable_form( $(this), 'gallery' );
		...
		validation_failure: function(){
			...
			$('.fieldmaster-gallery-side-data').each(function(){
				...
				fieldmaster.enable_form( $(this), 'gallery' );
...
// @codekit-prepend "../js/fieldmaster-repeater.js";
// @codekit-prepend "../js/fieldmaster-flexible-content.js";
// @codekit-prepend "../js/fieldmaster-gallery.js";
