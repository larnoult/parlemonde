...
/*
*  fieldmaster_get_field_reference()
*
...
function fieldmaster_get_field_reference( $field_name, $post_id ) {
	
	// vars
	$field_key = fieldmaster_get_metadata( $post_id, $field_name, true );
	
	
	// filter
	$field_key = apply_filters('fieldmaster/get_field_reference', $field_key, $field_name, $post_id);
...
function get_field( $selector, $post_id = false, $format_value = true ) {
	...
	// get field
	$field = fieldmaster_maybe_get_field( $selector, $post_id );
	...
	// get value for field
	$value = fieldmaster_get_value( $post_id, $field );
	...
	if( $format_value ) {
		...
		// get value for field
		$value = fieldmaster_format_value( $value, $post_id, $field );
...
function get_field_object( $selector, $post_id = false, $format_value = true, $load_value = true ) {
	...
	if( $format_value ) {
		...
		// get value for field
		$field['value'] = fieldmaster_format_value( $field['value'], $post_id, $field );
...
function get_field_objects( $post_id = false, $format_value = true, $load_value = true ) {
	...
	foreach( $meta as $k => $v ) {
		...
		$field = fieldmaster_maybe_get_field( $field_key );
...
function have_rows( $selector, $post_id = false ) {
	...
	$active_loop = fieldmaster_get_loop('active');
	$previous_loop = fieldmaster_get_loop('previous');
	...
	// loop has changed
		...
		$sub_field = fieldmaster_get_sub_field($selector, $active_loop['field']);
		...
		if( $change == 'post_id' ) {
			...
			if( empty($_post_id) && $sub_exists ) {
				...
				fieldmaster_remove_loop('active');
	...
	if( $new_parent_loop ) {
		...
		$value = fieldmaster_extract_var( $field, 'value' );
...
function the_row( $format = false ) {
	...
	// vars
	$i = fieldmaster_get_loop('active', 'i');
	...
	// update
	fieldmaster_update_loop('active', 'i', $i);
...
function get_row( $format = false ) {
	...
	// vars
	$loop = fieldmaster_get_loop('active');
	...
	// get value
	$value = fieldmaster_maybe_get( $loop['value'], $loop['i'] );
	...
	if( $format ) {
		...
		$value = fieldmaster_format_value( $loop['value'], $loop['post_id'], $loop['field'] );
		
		
		// get value
		$value = fieldmaster_maybe_get( $value, $loop['i'] );
...
function get_row_sub_field( $selector ) {
	...
	// vars
	$row = fieldmaster_get_loop('active');
	...
	// attempt to find sub field
	$sub_field = fieldmaster_get_sub_field($selector, $row['field']);
...
function get_sub_field_object( $selector, $format_value = true, $load_value = true ) {
	...
	if( $format_value ) {
		...
		// get value for field
		$sub_field['value'] = fieldmaster_format_value( $sub_field['value'], $row['post_id'], $sub_field );
...
function get_row_layout() {
	...
	// return
	if( isset($row['fieldmaster_fc_layout']) ) {
		
		return $row['fieldmaster_fc_layout'];
...
function fieldmaster_shortcode( $atts ) {
...
add_shortcode('fieldmaster', 'fieldmaster_shortcode');
...
function update_field( $selector, $value, $post_id = false ) {
	...
	// get field
	$field = fieldmaster_maybe_get_field( $selector, $post_id, false );
	...
	// save
	return fieldmaster_update_value( $value, $post_id, $field );
...
function update_sub_field( $selector, $value, $post_id = false ) {
	...
	if( is_array($selector) ) {
		...
		$post_id = fieldmaster_get_valid_post_id( $post_id );
		$sub_field = fieldmaster_maybe_get_sub_field( $selector, $post_id, false );
		
	} else {
		
		$post_id = fieldmaster_get_loop('active', 'post_id');
		$sub_field = get_row_sub_field( $selector );
	...
	// update
	return fieldmaster_update_value( $value, $post_id, $sub_field );
...
function add_row( $selector, $row = false, $post_id = false ) {
	...
	// update value
	fieldmaster_update_value( $value, $post_id, $field );
...
function add_sub_row( $selector, $row = false, $post_id = false ) {
	...
	// get raw value
	$value = fieldmaster_get_value( $post_id, $sub_field );
...
function fieldmaster_filter_post_id( $post_id ) {
	
	return fieldmaster_get_valid_post_id( $post_id );
