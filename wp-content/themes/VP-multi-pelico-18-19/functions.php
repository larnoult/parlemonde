...
if (is_user_logged_in()){
	...
	$content="<a href='/wp-login.php'>Connectez-vous </a>pour accéder au contenu !";
}
//$content=$marker;
...
if (is_user_logged_in()){
	$shortcode = sprintf( // À compléter à la fin pour afficher les activités pertinentes, avec l'argument "action" adéquate
	    '[activity-stream user_id="%1$s" allow_posting=0 action=activity_update,publication_activity,mpp_media_upload]',
	    $marker
	);
$content=do_shortcode($shortcode);
}
...
/* ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/* ------------------------To display the Peliquestion button OR the comment question ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
/* ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/

/* Changes to be moved live */
...
function question_comment() {
    ...
    if ( is_user_logged_in() ) { // If user is connected, can play with 

	if (is_single()){ // If on a post or portfolio
		$member_type = bp_get_member_type( get_current_user_id() ); 
	  if (($member_type=="grand-explorateur")||($member_type=="l-equipe-par-le-monde")){
		echo '<div id="question-GE">?</div>';
	}
	  else { // parents or explorateur en herbe
		echo '<div id="comment-EH">...</div>';
	}
			
	}
	
/*	else { // display a fake button, without clicking possibilities
...
/* To change the WP admin footer: based on: https://wpchannel.com/wordpress/tutoriels-wordpress/modifier-footer-administration-wordpress/*/
function wpc_remove_footer_admin() {
	echo 'Pour obtenir de l\'aide, <strong><a href="http://prof.parlemonde.org/course/ecrire-des-web-reportages/" target="_blank">consultez le guide en suivant ce lien </a></strong>';
}
